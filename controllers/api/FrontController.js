 ...
 /*------------------ Supportive functions ---------------------------*/
  ...
  exports.getHomeContent = async (req, res) => {
    ...
    try {
      ...
      const story_sql = `SELECT stories.id, stories.user_name, stories.comments, stories.image, stories.created_at FROM stories WHERE status='1' ORDER BY id DESC`;
  ...
  exports.getPageContent = async (req, res) => {
    page_slug = req.params.id
   
    try {
  
      const page_sql = `SELECT * FROM pages WHERE slug=?`;
      var pages = await db.query(page_sql,[page_slug]);
      pages = await updateBannerPath(pages);
  ...
  exports.getCovers = async (req, res) => {
    ...
    try {
        ...
        const cat_sql = `SELECT id, title FROM cover_categories WHERE status='1' ORDER BY sequence ASC`;
  ...
  exports.getMyAccountInfo = async (req, res) => {
    ...
    try {
      ...
      if(userResult.length>0){
        ...
        const address_sql = `SELECT * FROM user_address WHERE user_id=? ORDER BY id DESC`;
        var addressResult = await db.query(address_sql, [user_id]);
  ...
  exports.apiContactUs = async (req, res) => {
   ...
   const currentDate = new Date().toISOString().slice(0, 19).replace("T", " ");
    try {
      ...
      const mailOptions = {
        ...
        html: `
        <h1>Dear Admin,</h1>
        <p>${name} was filled the contact form at ${currentDate}.</p>
        <p></p>
        <p>Name: ${name}</p>
        <p>Email: ${email}</p>
        <p>Subject: ${subject}</p>
      ...
      transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
              ...
              res.status(200).send({ 
                status: true, 
                result: { "message":"Thank you for contacting us. Our team will respond to you shortly." }, 
                errors: "" 
              });  
          }
      });
      ...
      /*res.status(200).send({ 
        status: true, 
        result: {message: "Thank you for contact us. Our team will be response you soon."}, 
        errors: "" 
      }); */
  ...
  exports.apiTiktokEvent = async (req, res) => {
    const TiktokData = req.body;
    const TiktokApiUrl = `https://business-api.tiktok.com/open_api/v1.3/event/track/`
    const TiktokHeaders= {
      'Access-Token': `${process.env.TIKTOK_ACCESSTOKEN}`,
      'Content-Type': 'application/json', // Include this header if needed
    }
    console.log("TiktokData->", TiktokApiUrl,TiktokHeaders, TiktokData)
    axios.post(TiktokApiUrl, JSON.stringify(TiktokData), { headers: TiktokHeaders})
      .then(response => {
        console.log("TiktokApiResponse->", response.data.message)
        res.status(200).send({ status: true, result: response.data.message, errors:'' });
      })
      .catch(error => {
        console.log("TiktokApiResponse->", error)
        res.status(500).send({ status: false, result: "", errors:error });
      });
  }

  exports.apiPinterestEvent = async (req, res) => {
    const PinterestData = req.body;
    const PinterestApiUrl = `https://api.pinterest.com/v5/ad_accounts/${process.env.PINTEREST_ACCOUNT}/events`
    const PinterestHeaders= {
      'Authorization': `Bearer ${process.env.PINTEREST_ACCESSTOKEN}`,
      'Content-Type': 'application/json', // Include this header if needed
    }
    console.log("PinterestData->", PinterestApiUrl,PinterestHeaders, PinterestData)
    axios.post(PinterestApiUrl, JSON.stringify(PinterestData), { headers: PinterestHeaders})
      .then(response => {
        console.log("PinterestApiResponse->", response.data.events)
        res.status(200).send({ status: true, result: response.data.events, errors:'' });
      })
      .catch(error => {
        console.log("PinterestApiResponse->", error)
        res.status(500).send({ status: false, result: "", errors:error });
      });
  }
