...
exports.index = async (req, res) => {    
    ...
    try {
        const sqlCount = 'SELECT COUNT(*) AS totalTags FROM `blog_tags`';
        const [countRows] = await db.query(sqlCount);
        const totalTags = countRows.totalTags;

        const sql = 'SELECT * FROM `blog_tags` LIMIT ? OFFSET ?';
        const tags = await db.query(sql, [perPage, offset]);
    
        res.render("BlogTag/index", {
            title: "Blogs Tags",
            tags: tags,
            baseUrl: baseUrl,
            paginationUrl:"/admin/blog-tags",
            currentPage: page,
...
exports.insert = async (req, res) => {    
    const { name } = req.body;
    try {
        const sql = "SELECT * FROM `blog_tags` WHERE name=?";
        const tag = await db.query(sql, [name]);
        if(tag.length === 0)
        {
            const sql = "INSERT INTO `blog_tags` SET name=?";
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        if(tag.length > 0)
        {
            const sql = "UPDATE `blog_tags` SET name=? WHERE id=?";
